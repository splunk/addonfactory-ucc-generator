...
table_sourcetype_query = (
    ...
    '| stats latest(_time) AS \\"Last event\\", sparkline(sum(n_events)) as sparkevent, '
    "sum(n_events) as events by sourcetype_ingested "
    '| rename sourcetype_ingested as st ] | makemv delim=\\",\\" sparkevent | convert ctime(\\"Last event\\") '
...
table_source_query = (
    ...
    '| rename modular_input_name as s ] | makemv delim=\\",\\" sparkevent | convert ctime(\\"Last event\\") '
...
table_host_query = (
    ...
    '| convert ctime(\\"Last event\\") | table h, Bytes, sparkvolume '
...
table_index_query = (
    ...
    '| rename event_index as idx ] | makemv delim=\\",\\" sparkevent | convert ctime(\\"Last event\\") '
...
table_account_query = (
    "index = _internal source=*{addon_name}* action=events_ingested "
    "| stats latest(_time) as ls, sparkline(sum(n_events)) as sparkevent, sum(n_events) as events by event_account "
    "| convert ctime(ls) | table event_account, events, sparkevent, ls "
    '| rename event_account as \\"Account\\", events as \\"Number of events\\", '
    'sparkevent as \\"Event trendline\\", ls as \\"Last event\\"'
)
table_input_query = (
    "index = _internal source=*{addon_name}* action=events_ingested "
    "| stats latest(_time) as ls, sparkline(sum(n_events)) as sparkevent, sum(n_events) as events by event_input "
    "| convert ctime(ls) | table event_input, events, sparkevent, ls "
