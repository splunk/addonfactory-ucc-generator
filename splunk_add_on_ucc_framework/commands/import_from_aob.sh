...
do
    ...
    sed -i '' '/^import .*_declare$/d' $OUTPUT
    sed -i '' '/^from solnlib.packages.splunklib import modularinput as smi/d' $OUTPUT
    sed -i '' '/import modinput_wrapper.base_modinput/d' $OUTPUT
    sed -i '' '/Do not edit this file!!!/,/Add your modular input logic to file/d' $OUTPUT

    # -------------------------------------------------------------------------------
    # change the reference for base_modinput to use the name in the imports.py
    # -------------------------------------------------------------------------------
    sed -i '' 's/(modinput_wrapper.base_modinput./(base_mi./' $OUTPUT
    ...
    sed -i '' "/^        if 'use_single_instance_mode' /,/use_single_instance = False/{/use_single_instance = False/p;d;}" $OUTPUT
    sed -i '' 's/^            use_single_instance = False/        use_single_instance = False/' $OUTPUT
