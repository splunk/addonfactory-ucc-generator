...
#original_app_manifest=$(cat ./package/app.manifest | sed '/^#/d' )
#
#echo $original_app_manifest
#
#if [[ ! $original_app_manifest == *"supportedDeployments"* ]] || [[ $original_app_manifest == *'"supportedDeployments": null'* ]]; then
#  sup_dep_part='"supportedDeployments": ["*","_standalone","_distributed","_search_head_clustering"],'
#fi
#
#if [[ ! $original_app_manifest == *"targetWorkloads"* ]] || [[ $original_app_manifest == *'"targetWorkloads": null'* ]]; then
#  tar_work_part='"targetWorkloads":     ["*","_search_heads","_indexers","_forwarders"],'
#fi
#
#new_schema_ver='"schemaVersion": "2.0.0",'
#app_manifest_content="$new_schema_ver $sup_dep_part $tar_work_part"
#echo
#echo $app_manifest_content
#
#sed -i'.bak' -E -e "s~\"schemaVersion\": \"([^\"]+)\",~$app_manifest_content~" -e s~ ./package/app.manifest
