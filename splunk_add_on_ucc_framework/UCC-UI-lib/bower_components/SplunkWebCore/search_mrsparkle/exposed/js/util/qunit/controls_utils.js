...
define(['jquery',
    ...
    var BaseControlModule = {
        ...
        verifyControlEnabled: function(controlView, model, modelAttribute) {
            ...
            ok(true, 'testing the ' + modelAttribute + ' control');
            if(controlView instanceof TextControl || controlView instanceof TextareaControl) {
                ok(true, 'setting a value in the text box');
                this.writeControlValue(controlView, 'foo');
                equal(model.get(modelAttribute), 'foo', 'the model was updated correctly');
                ok(true, 'setting a value in the model');
                model.set(modelAttribute, 'bar');
                equal(this.readControlValue(controlView), 'bar', 'the text box was updated correctly');
            }
                ...
                ok(true, 'clicking the ' + values[0] + ' button');
                this.writeControlValue(controlView, values[0]);
                equal(model.get(modelAttribute), values[0], 'the model was updated correctly');
                ok(true, 'setting the model attribut to ' + values[1]);
                model.set(modelAttribute, values[1]);
                equal(this.readControlValue(controlView), values[1], 'the radio control was updated correctly');
            }
            else {
                throw new Error('Unsupported control type');
            }
        ...
        mainRun: function() {
            this.setupControl({});
            ok(this.controlView, "Simple Control created with no options");
            this.cleanup();
            ...
            equal(this.controlView.getValue(), "newDefaultValue","Default value assigned");
            // model/modelAttribute overrides defaultValue
            this.setupDefaultControl({defaultValue:"newDefaultValue"}); // Setup with model
            equal(this.controlView.getValue(), "stringAttrValue","Default value overridden by model attribute value");
            this.controlView.setValue("overrideValue");
            equal(this.controlView.getValue(), "overrideValue","Set value called");
            this.cleanup();

            // ENABLED
            this.setupDefaultModel();
            this.setupControl({enabled:false});
            ok(this.controlView, "View was created with enabled = false");
            this.cleanup();

            // VALIDATE = FALSE
            ok(true, "Test validate = false");
            this.setupValidatingModel();
            this.setupControl({validate:true, model:this.model, modelAttribute:"typeAttr"});
            equal(this.controlView.getValue(), "", "Value is empty string");
            this.controlView.setValue("InvalidType");
            equal(this.controlView.getValue(), "InvalidType", "Set value to InvalidType. Value should change");
            equal(this.model.get("typeAttr"), "", "Model attribute should remain empty string");
            this.cleanup();

            // VALIDATE = TRUE
            ok(true, "Test validate = true");
            this.setupValidatingModel();
            this.setupControl({forceUpdate:true, validate:true, model:this.model, modelAttribute:"typeAttr"});
            equal(this.controlView.getValue(), "", "Value is empty string");
            this.controlView.setValue("InvalidType");
            equal(this.controlView.getValue(), "InvalidType", "Set value to InvalidType. Value should change");
            equal(this.model.get("typeAttr"), "InvalidType", "Model attribute should change");
            this.cleanup();

            // UPDATEMODEL
            ok(true, "Test updateModel");
            this.setupDefaultModel();
            this.setupControl({updateModel:false, model:this.model, modelAttribute:"stringAttr"});
            equal(this.controlView.getValue(), "stringAttrValue", "Value set by model");
            this.controlView.setValue("updatedStringAttrValue");
            equal(this.controlView.getValue(), "updatedStringAttrValue", "Set value. Value should change");
            equal(this.model.get("stringAttr"), "stringAttrValue", "Model attribute should not change");
            this.cleanup();

            // GET MODEL ATTRIBUTE
            ok(true, "Test getModelAttribute");
            this.setupDefaultModel();
            this.setupDefaultControl();
            equal(this.controlView.getModelAttribute(), 'stringAttr', 'getModelAttribute returns the correct value');
            this.cleanup();

            // NORMALIZE VALUE
            ok(true, "Test normalizeValue");
            this.setupDefaultModel();
            this.setupDefaultControl();
            equal(this.controlView.normalizeValue("SomeValue"), 'SomeValue', 'normalizeValue returns the same value');
            this.cleanup();

            // UPDATE VALUE
            ok(true, "Test updateValue");
            this.setupControl();
            equal(this.controlView.getValue(), "", 'Empty value');
            this.controlView.setValue("UpdateValue");
            equal(this.controlView.getValue(), "UpdateValue", 'setValue called. Value should change');
            this.cleanup();

            // UNSETVALUE
            ok(true, "Test unsetValue");
            this.setupDefaultModel();
            this.setupDefaultControl();
            this.controlView.unsetValue();
            equal(this.model.get(this.controlView.getModelAttribute()), undefined, 'unsetValue sets value to undefined');
            this.cleanup();
            ...
            equal(this.model.get(this.controlView.getModelAttribute()), "", 'Model attribute value in model');
            this.controlView._value = "UpdatedTypeValue";
            this.controlView.updateModel();
            equal(this.controlView.getValue(), "UpdatedTypeValue", 'updateModel called. Value should change');
            this.cleanup();

            // GET UPDATED MODEL ATTRIBUTES
            ok(true, "Test getUpdatedModelAttributes");
            this.setupValidatingModel();
            this.setupControl({model:this.model, modelAttribute:"typeAttr"});
            equal(this.controlView.getValue(), "", 'Empty value');
            this.controlView._value = "UpdatedTypeValue";
            deepEqual(this.controlView.getUpdatedModelAttributes(), {typeAttr:"UpdatedTypeValue"}, 'getUpdatedModelAttributes called.');
            },
