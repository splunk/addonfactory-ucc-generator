define([
    ...
    var BaseModule = {
        ...
        setup: function() {
            ...
            ok(true, 'Chart testing module setup successful');
        },
        ...
        drawChart: function(data, props, callback) {
            ...
            stop();
            return this.chart.prepareAndDraw(jsCharting.extractChartReadyData(data), props, _(function(chart) {
        ...
        updateInPlace: function(newData, newProps, callback, options) {
            ...
            this.chart.prepareAndDraw(dataSet, newProps || {}, function(chart) {
                if(options.expectRedrawInPlace !== false) {
                    equal(that.chart.redrawInPlace.callCount, 1, 'redrawInPlace was called');
                }
                else {
                    equal(that.chart.redrawInPlace.callCount, 0, 'redrawInPlace was not called');
                }
        ...
        withAllTestWidths: function(data, props, callback) {
            _(this.testWidths).each(function(width) {
                ...
                ok(true, 'testing with width = ' + width);
        ...
        withAllTestHeights: function(data, props, callback) {
            _(this.testHeights).each(function(height) {
                ...
                ok(true, 'testing with height = ' + height);
                this.drawChart(data, props, function(hcChart) {
                    callback.call(this, hcChart);
                });
            }, this);
        ...
        assertLegendItems: function(chart, expected, message) {
            ...
            deepEqual(items, expected, message || 'legend items are correct');
        ...
        assertTooltipOffset: function(pointIndex, seriesIndex, hcChart, expected, message) {
            ...
            if(expected.hasOwnProperty('top')) {
                QUnit.assert.close(adjustedOffset.top, expected.top, tolerance, 'top: ' + message);
            }
            if(expected.hasOwnProperty('left')) {
                QUnit.assert.close(adjustedOffset.left, expected.left, tolerance, 'left: ' + message);
            }
            this.simulatePointMouseOut(hcChart, seriesIndex, pointIndex);
        },
        ...
        assertAxisLabels: function(axis, expected, message) {
            ...
            deepEqual(labels, expected, message || (axis.coll + ' axis labels are correct'));
        },

        assertAxisTitle: function(axis, expected, message) {
            var title = axis.axisTitle.textStr;

            deepEqual(title, expected, message || (axis.coll + ' axis titles are correct'));
        },
        ...
        assertVerticallyCenteredLabel: function(tick, message) {
            ...
            QUnit.assert.close(labelCenter, boundsCenter, 2, message || 'tick ' + tick.pos + ' is vertically centered correctly');
        },
        ...
        assertTopAlignedLabel: function(tick, message) {
            ...
            QUnit.assert.close(bounds.top + this.verticalTickLabelOffset, labelTop, 2, message);
        },
        ...
        assertHorizontalLabelAlignment: function(tick, tickDescription, expectedLabelOffset) {
            ...
            if(this.isTimeAxis(tick.axis)) {
                QUnit.assert.close(left, boundsLeft + tickLabelOffset, 2.5,
                    tickDescription + ' has the correct left alignment');
            }
            else if (this.isNumericAxis(tick.axis)){
                QUnit.assert.close(labelCenter, boundsLeft, 3,
                        tickDescription + ' has the correct left alignment');
            }
            else {
                if(this.axisHasTickmarksBetween(tick.axis)){
                    ...
                    QUnit.assert.close(labelCenter, areaCenter, 3,
                                            tickDescription + ' has the correct left alignment');
                }
                else {
                    QUnit.assert.close(labelCenter, boundsLeft, 3,
            ...
            ok(right <= boundsRight + 1, tickDescription + ' does not overflow its boundary');
        ...
        verifyShapeAttrs: function(wrapper, attrs, messagePrefix) {
            ...
            if(attrs.fillColor) {
                ...
                if (isEqual) {
                    equal (true, isEqual, messagePrefix + ' has the correctFill color');
                } else {
                    equal($element.attr('fill'), attrs.fillColor, messagePrefix + ' has the correct fill color');
            ...
            if(attrs.strokeColor) {
                ...
                if (isEqual) {
                    equal (true, isEqual, messagePrefix + ' has the correct stroke color');
                } else {
                    equal(attrs.strokeColor,$element.attr('stroke'), messagePrefix + ' has the correct stroke color');
                }
            }
            ...
            if(attrs.hasOwnProperty('strokeOpacity')) {
                ...
                equal(parseFloat(elementStrokeOpacity) || 1, attrs.strokeOpacity, ' has the correct stroke opacity');
            }
        ...
        verifyPointMarkerActive: function(hcChart, seriesIndex, pointIndex, seriesColor) {
            ...
            ok(marker.attr('visibility'), 'the point marker is visible');
            equal(marker.attr('fill'), seriesColor, 'the marker has the correct color');
            QUnit.assert.close(
                markerOffset.left,
        ...
        verifyPointMarkerInactive: function(hcChart, seriesIndex) {
            ...
            ok(!marker || marker.attr('visibility') === 'hidden', 'the point marker is not visible');
        },
        ...
        verifyColumnSeriesInactive: function(hcChart, seriesIndex) {
            ...
            ok(!series.splSeries.selectedPointGraphic, 'the active point graphic was removed');
        },
        ...
        verifyColumnPointActive: function(hcChart, seriesIndex, pointIndex, seriesColor) {
            ...
            equal(selectedPointGraphic.attr('x'), activePointShapeArgs.x, 'the active graphic x value is correct');
            equal(selectedPointGraphic.attr('y'), activePointShapeArgs.y, 'the active graphic y value is correct');
            equal(selectedPointGraphic.attr('width'), activePointShapeArgs.width, 'the active graphic width value is correct');
            equal(selectedPointGraphic.attr('height'), activePointShapeArgs.height, 'the active graphic height value is correct');
            equal(selectedPointGraphic.attr('zIndex'), 1, 'the active graphic z-index is correct');
        },
        ...
        verifyBubblePointActive: function(hcChart, seriesIndex, pointIndex, seriesColor) {
            var series = hcChart.series[seriesIndex],
                ...
                markerOffset = $(marker.element).offset(),
                chartOffset = $(hcChart.container).offset();
            ok(series.group.attr('visibility'), 'the point marker & its series are visible');
            this.verifyShapeAttrs(point.graphic, {
                fillColor: seriesColor,
                fillOpacity: 0.5,
                strokeColor: seriesColor,
        ...
        fullPointHoverTest: function(hcChart, colors) {
            ...
            _(hcChart.series).each(function(series, i) {
                _(series.data).each(function(point, j) {
                    ok(true, 'simulating a hover event on series ' + (i + 1) + ' point ' + (j + 1));
                    this.simulatePointMouseOver(hcChart, i, j);
                    ok(!hcChart.tooltip.isHidden, 'the tooltip is visible');
                    this.verifyPointHoverState(hcChart, i, j, colors[i]);
                    this.simulatePointMouseOut(hcChart, i, j);
                    ok(hcChart.tooltip.isHidden, 'the tooltip is hidden');
                }, this);
            }, this);
        },
        ...
        teardown: function() {
            ...
            ok(true, 'Chart testing module teardown successful');
        },
