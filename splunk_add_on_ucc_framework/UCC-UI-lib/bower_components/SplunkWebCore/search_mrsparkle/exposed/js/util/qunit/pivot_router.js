    ...
    var BaseModule = _.extend({}, qunitUtils.FakeXhrModule, {
        ...
        setup: function() {
            ...
            this.populateClassicurl = function(uriAttrs) {
                ok(true, 'populating query string params with ' + JSON.stringify(uriAttrs));
            ...
            this.verifyDefaultPivotConfig = function(options) {
                options = options || {};
                equal(this.report.getPivotObjectName(), OBJECT_NAME, 'the object name is correct');
                equal(this.report.getNumCells(), 1, 'a cell element is in the report');
                var cell = this.report.entry.content.cells.at(0);
                deepEqual(cell.pick('displayName', 'fieldName', 'owner', 'type'), {
                ...
                equal(this.report.getNumRows(), 0, 'no row elements are in the report');
                equal(this.report.getNumColumns(), 0, 'no column elements are in the report');
                if(options.checkFilters) {
                    equal(this.report.getNumFilters(), 0, 'no filter elements are in the report');
                }
            };
            ...
            this.verifyCustomPivotConfig = function(options) {
                ...
                equal(this.report.getNumRows(), 1, 'a cell element is in the report');
                var row = this.report.entry.content.rows.at(0);
                deepEqual(row.pick('displayName', 'fieldName', 'owner', 'type'), {
                    displayName: 'clientip',
                    fieldName: 'clientip',
                    owner: OBJECT_NAME,
                    type: 'ipv4'
                ...
                equal(this.report.getNumColumns(), 1, 'a cell element is in the report');
                var column = this.report.entry.content.columns.at(0);
                deepEqual(column.pick('displayName', 'fieldName', 'owner', 'type'), {
                    displayName: 'useragent',
                    fieldName: 'useragent',
                    owner: OBJECT_NAME,
                    type: 'string'
            ...
            this.verifyAndRespondToCollect = function(expectedDispatchAttrs, customResponseAttrs, options) {
                ...
                ok(true, 'responding to the collect dispatch');
            ...
            this.verifyCollectJobNotPaused = function(sid) {
                ...
                ok(!pauseRequest, 'pause was not called on the collect job');
            };
            ...
            this.verifyCollectJobPaused = function(sid) {
                ...
                ok(pauseRequest, 'pause was called on the collect job');
            ...
            this.verifyCollectJobNotUnpaused = function(sid) {
                ...
                ok(!unpauseRequest, 'unpause was not called on the collect job');
            };
            ...
            this.verifyCollectJobUnpaused = function(sid) {
                ...
                ok(unpauseRequest, 'unpause was called on the collect job');
                this.respondTo(unpauseRequest, {});
            };

            this.verifyCollectJobNotPolling = function() {
                sinon.stub(this.collectJob, 'safeFetch');
                ok(true, 'waiting for the collect job poller');
                this.clock.tick(1000);
                equal(this.collectJob.safeFetch.callCount, 0, 'the collect job is not polling');
            ...
            this.verifyCollectJobPolling = function() {
                ...
                ok(true, 'waiting for the collect job poller');
                this.clock.tick(1000);
                ok(this.collectJob.safeFetch.callCount > 0, 'the collect job is polling');
                this.collectJob.safeFetch.restore();
            };

            this.verifyPivotSearchFetch = function(request, options) {
                ok(request.url.indexOf(NORMALIZED_DATA_MODEL_ID.replace('datamodeleai', 'datamodelreport') > -1),
            ...
            this.verifyPivotJobNotDispatched = function() {
                equal(this.pivotJob.sync.createSpy.callCount, 0, 'pivot job has not been dispatched');
            };
            ...
            this.verifyFinalReportState = function(timeRange, options) {
                ...
                if(this.objectHasIndexTime) {
                    equal(this.report.getNumFilters(), 1, 'a filter element is in the report');
                    ...
                    deepEqual(filter, expectedAttrs, 'the filter attributes are correct');
                }
                else {
                    equal(this.report.getNumFilters(), 0, 'there are no filter elements is in the report');
                }
                equal(this.report.entry.content.get('search'), this.pivotSearch.get('pivotSearch'),
                ...
                deepEqual(this.pivotJob.save.lastCall.args[1].data, expectedPivotJobParams,
            ...
            this.quickSetupNewReport = function(uriParams) {
                ok(true, 'setting up a new report');
            ...
            this.quickSetupExistingReport = function(uriParams) {
                ok(true, 'setting up an existing report');
            ...
            ok(true, 'module setup successful');
        },
        ...
        teardown: function() {
            ...
            ok(true, 'module teardown successful');
        }
