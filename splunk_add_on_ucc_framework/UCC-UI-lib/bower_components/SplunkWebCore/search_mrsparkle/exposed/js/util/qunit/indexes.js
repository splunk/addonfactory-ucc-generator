...
define([
    ...
    return {
        ...
        testSortColumn: function(sortLink, sortKey, indexesFetchData, isHidden) {
            ...
            if (!isHidden){
                equal(sortKeyValue, sortKey, "Sort " + sortKey + " column");
            }
            else {
                equal(sortKeyValue, "", "Did Not Sort " + sortKey + " column because column is hidden");
            }
        },
        ...
        testClickLink: function(linkClass, linkEvent, controllerModel) {
            ...
            this.clock.tick(1);
            equal(linkPressed, true, linkClass + " Link clicked");
        },
        ...
        testCreateNewIndex: function(controllerModel){
            ...
            equal(createIndexClicked, true, "New Index Button clicked");
        },
        ...
        testNameFilter: function(searchString, view, indexesFetchData){
            ...
            equal(nameFilterValue, searchString, "Name Filter value updated");
        },
        ...
        testCreateIndex: function(indexName, values, view, model){
            ...
            if (indexName){
                ...
                if (this.product === 'core'){
                    ...
                    model.on("change:maxTotalDataSizeGB", function(indexModel, value) {
                        ok(value, "maxTotalDataSizeGB set to " + value);
                    }, this);
                    model.on("change:frozenTimePeriodInDays", function(indexModel, value) {
                        ok(value, "frozenTimePeriodInDays set to " + value);
                    }, this);
            ...
            if (model.set({}, {validate:true})) {
                equal(name, indexName, "New index (" + indexName + ") created.");
            }
            else {
                ok(!name, "New index not created. Name is required.");
            }
        },
