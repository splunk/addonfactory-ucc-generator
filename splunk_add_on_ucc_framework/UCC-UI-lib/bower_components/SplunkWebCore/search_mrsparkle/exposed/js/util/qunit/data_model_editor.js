define([
    ...
    var setupHelper = function() {
        ...
        this.verifySampleSizeWiring = function(addEditView, $container) {
            equal($container.find('.control[data-name="sampleSize"]').length, 1, 'one is a sample size control');
            ok(true, 'simulating changing the sample size drop-down');
        ...
        this.verifyPageSizeWiring = function(addEditView, $container) {
            equal($container.find('.control[data-name="count"]').length, 1, 'one is a page size control');
            ok(true, 'simulating changing the page size drop-down');
            ...
            equal(resultsFetchData.get('count'), 100, 'the page count was updated');
        };

        this.verifyClusterFilterWiring = function(addEditView, $container) {
            equal($container.find('.control[data-name="filter"]').length, 1, 'one is a filter control');
            equal($container.find('.control[data-name="clustering"]').length, 1, 'one is a clustering control');
            var stateModel = addEditView.children.formView.model.state;

            ok(true, 'simulating changing the filter text box');
            ...
            equal(stateModel.get('filter'), 'my-test-filter', 'the model was updated');

            ok(true, 'simulating changing the clustering drop-down');
            this.writeSyntheticSelectValue($container.find('.control[data-name="clustering"]'), fieldExtractorUtils.CLUSTERING_DIVERSE);
            equal(stateModel.get('clustering'), fieldExtractorUtils.CLUSTERING_DIVERSE, 'the model was updated');
        };

        this.verifyEventsViewWiring = function(addEditView, $container) {
            equal($container.find('.control[data-name="eventsView"]').length, 1, 'one is an eventsView control');
            var stateModel = addEditView.children.formView.model.state;
            ok(true, 'simulating changing the eventsView radio');
            this.writeSyntheticRadioValue($container.find('.control[data-name="eventsView"]'), fieldExtractorUtils.VIEW_MATCHING_EVENTS);
            equal(stateModel.get('eventsView'), fieldExtractorUtils.VIEW_MATCHING_EVENTS, 'the model was updated');
        };
        ...
        this.verifyPageSizeAndSampleSizeOnly = function(addEditView, tabContent) {
            ...
            equal($controlsRow.find('.control').length, 1, 'there are two controls in the control row');
            this.verifySampleSizeWiring(addEditView, $controlsRow);

            var $jobStatusRow = this.getJobStatusRowFromTabContent(tabContent);
            equal($jobStatusRow.find('.control').length, 1, 'there is one control in the job status bar');
            this.verifyPageSizeWiring(addEditView, $jobStatusRow);
        };

        this.verifyClusterFilterControls = function(addEditView, tabContent) {
            var $controlsRow = this.getControlsRowFromTabContent(tabContent);
            equal($controlsRow.find('.control').length, 3, 'there are three controls in the control row');
            this.verifySampleSizeWiring(addEditView, $controlsRow);
        ...
        this.verifyClusterFilterMatchControls = function(addEditView, tabContent) {
            var $controlsRow = this.getControlsRowFromTabContent(tabContent);
            equal($controlsRow.find('.control').length, 4, 'there are four controls in the control row');
            this.verifySampleSizeWiring(addEditView, $controlsRow);
            this.verifyClusterFilterWiring(addEditView, $controlsRow);
