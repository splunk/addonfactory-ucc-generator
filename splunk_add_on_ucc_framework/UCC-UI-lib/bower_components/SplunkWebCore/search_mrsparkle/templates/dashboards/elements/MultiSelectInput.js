...
for opt in ('valuePrefix', 'valueSuffix', 'delimiter', 'allowCustomValues'):
    if hasattr(element, opt) and getattr(element, opt) is not None:
        options[opt] = getattr(element, opt)
