package com.splunk.services.search
{
	...
	/**
	 * Dispatched after the load operation is complete and results data is
	 * available.
	 * 
	...
	/**
	 * The SearchJobResults class is used to retrieve the transformed events
	 * from an existing search job.
	 */
	public class SearchJobResults extends EventDispatcher
	{

		// Private Properties

		private var _job:SearchJob;
		private var _data:ResultsData;
		...
		/**
		 * Creates a new SearchJobResults object.
		 * 
		 * @param job The SearchJob instance from which to retrieve results.
		 * 
		 * @throws TypeError The value of the job parameter is null.
		 * 
		 * @see SearchJob
		 */
		public function SearchJobResults(job:SearchJob)
		{
			if (!job)
		...
		/**
		 * The SearchJob instance that this SearchJobResults object connects to.
		...
		/**
		 * The results data received from the load operation. This property is
		 * populated only when the load operation is complete.
		 * 
		 * @see com.splunk.services.search.data.ResultsData
		 */
		public function get data() : ResultsData
		{
			return this._data;
		}

		/**
		 * The length of time, in seconds, between automatic reloads of results.
		 * If set to zero or less, automatic reloads are disabled.
		 * 
		 * @default 0
		 * 
		 * @see #refresh()
		...
		/**
		 * Loads results from the associated SearchJob instance. If results are
		 * already loading, the load operation is closed.
		 * 
		 * @param request A ResultsRequest object specifying the options for
		 * loading results.
		 * 
		 * @see com.splunk.services.search.request.ResultsRequest
		 */
		public function load(request:ResultsRequest = null) : void
		{
			if (!request)
		...
		/**
		 * Reloads results from the associated SearchJob instance. If results
		 * are already loading, the load operation is closed.
		 * 
		 * @see #refreshInterval
		 */
		...
		private function _load_complete(e:ServiceEvent) : void
		{
			...
			if (response.status == 200)
			{
				try
				{
					var data:ResultsData = new ResultsData(response.data, (this._loadRequest.variables.show_offset == 1));
