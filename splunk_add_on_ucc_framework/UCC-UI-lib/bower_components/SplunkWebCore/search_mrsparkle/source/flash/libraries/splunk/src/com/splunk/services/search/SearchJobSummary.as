package com.splunk.services.search
{
	...
	import com.splunk.services.search.data.SummaryData;
	import com.splunk.services.search.request.SummaryRequest;
	...
	/**
	 * Dispatched after the load operation is complete and summary information
	 * is available.
	 * 
	...
	/**
	 * The SearchJobSummary class is used to retrieve summary information of the
	 * untransformed events from an existing search job.
	 */
	public class SearchJobSummary extends EventDispatcher
	{

		// Private Properties

		private var _job:SearchJob;
		private var _data:SummaryData;
		...
		/**
		 * Creates a new SearchJobSummary object.
		 * 
		 * @param job The SearchJob instance from which to retrieve summary
		 * information.
		 * 
		 ...
		 * @see SearchJob
		 */
		public function SearchJobSummary(job:SearchJob)
		{
			if (!job)
		...
		/**
		 * The SearchJob instance that this SearchJobSummary object connects to.
		 * This value corresponds to the value of the job parameter specified in
		 * the constructor.
		 * 
		 * @see SearchJob
		 */
		...
		/**
		 * The summary information received from the load operation. This
		 * property is populated only when the load operation is complete.
		 * 
		 * @see com.splunk.services.search.data.SummaryData
		 */
		public function get data() : SummaryData
		{
			return this._data;
		}

		/**
		 * The length of time, in seconds, between automatic reloads of summary
		...
		/**
		 * Loads summary information from the associated SearchJob instance. If
		 * summary information is already loading, the load operation is closed.
		 * 
		 * @param request A SummaryRequest object specifying the options for
		 * loading summary information.
		 * 
		 * @see com.splunk.services.search.request.SummaryRequest
		 */
		public function load(request:SummaryRequest = null) : void
		{
			if (!request)
		...
		/**
		 * Reloads summary information from the associated SearchJob instance.
		 * If summary information is already loading, the load operation is
		 * closed.
		 * 
		 * @see #refreshInterval
		 */
		...
		private function _load_complete(e:ServiceEvent) : void
		{
			...
			if (response.status == 200)
			{
				try
				{
					var data:SummaryData = new SummaryData(response.data);
