package com.splunk.services.search
{
	...
	/**
	 * Dispatched after the load operation is complete and events data is
	 * available.
	 * 
	...
	/**
	 * The SearchJobEvents class is used to retrieve the raw untransformed
	 * events from an existing search job.
	 */
	public class SearchJobEvents extends EventDispatcher
	{

		// Private Properties

		private var _job:SearchJob;
		private var _data:EventsData;
		...
		/**
		 * Creates a new SearchJobEvents object.
		 * 
		 * @param job The SearchJob instance from which to retrieve events.
		 * 
		 * @throws TypeError The value of the job parameter is null.
		 * 
		 * @see SearchJob
		 */
		public function SearchJobEvents(job:SearchJob)
		{
			if (!job)
		...
		/**
		 * The SearchJob instance that this SearchJobEvents object connects to.
		...
		/**
		 * The events data received from the load operation. This property is
		 * populated only when the load operation is complete.
		 * 
		 * @see com.splunk.services.search.data.EventsData
		 */
		public function get data() : EventsData
		{
			return this._data;
		}

		/**
		 * The length of time, in seconds, between automatic reloads of events.
		...
		/**
		 * Loads events from the associated SearchJob instance. If events are
		 * already loading, the load operation is closed.
		 * 
		 * @param request An EventsRequest object specifying the options for
		 * loading events.
		 * 
		 * @see com.splunk.services.search.request.EventsRequest
		 */
		public function load(request:EventsRequest = null) : void
		{
			if (!request)
		...
		/**
		 * Reloads events from the associated SearchJob instance. If events are
		 * already loading, the load operation is closed.
		 * 
		 * @see #refreshInterval
		 */
		...
		private function _load_complete(e:ServiceEvent) : void
		{
			...
			if (response.status == 200)
			{
				try
				{
					var data:EventsData = new EventsData(response.data, (this._loadRequest.variables.show_offset == 1));
