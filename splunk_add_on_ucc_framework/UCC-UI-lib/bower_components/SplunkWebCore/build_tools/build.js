...
//SPL-130518
//On Solaris listing the network interfaces is disabled. Newer versions of the 'uniqid'
//package (> 1.0.0) use the 'macaddress' package, which calls os.networkInterfaces internally,
//causing a subsequent node crash. To prevent that from happening the method is patched
//here to return an empty interface list instead.
//TODO: remove once we no longer build the UI on Solaris (SPL-131456)
...
function getProdDebugConfig(configPath) {
    var paths = [
        path.join(__dirname, 'profiles', 'common', 'prod_debug.config.js')
    ]
