...
var collectFilesSync = require('./build_tools/util/collectFilesSync');
...
if (args.jobs == null) {
    if (args.runOnce) {
        ...
        logger.log('CPU cores:', cores, '--> -j' + args.jobs);
...
try {
    if (args.runOnce && ((fs.lstatSync(constants.jsBuildDir).isDirectory() && !args.appDir) || (fs.lstatSync(path.join(args.appDir, 'appserver', 'static', 'build')).isDirectory() && args.appDir))) {
        logger.log("Minification already run once");
        process.exit(0);
    }
} catch (e) {
...
var buildTasks = [];
if (args.appDir) {
    var profileDirPath = path.resolve(args.appDir, 'profiles');
    var buildJsProfilesTasks = tasks.generateJsProfilesTasks(profileDirPath, args.appName, args);
    var lessPagesDirPath = path.join(args.appDir, 'src', 'less', 'pages');
    var cssPagesDirPath = path.join(args.appDir, 'appserver', 'static', 'build', 'css', 'pages');
    var buildCoreLessPagesTasks = tasks.generateCoreLessPagesTasks(lessPagesDirPath, cssPagesDirPath, undefined, args.appName);

    buildTasks = concatAll(
        buildCoreLessPagesTasks,
        buildJsProfilesTasks
    );
} else {
    // Check dependencies

    if (!process.env.SPLUNK_SOURCE) {
        logger.log("Need to set $SPLUNK_SOURCE or $SPLUNK_SRC or pass the -s option");
        process.exit(1);
    }
    if (!process.env.SPLUNK_HOME) {
        logger.log("Need to set $SPLUNK_HOME or pass the -H option");
        process.exit(1);
    }

    var profileDirPath = path.join(constants.webBuildDir, 'build_tools', 'profiles');
    buildTasks = tasks.generateJsProfilesTasks(profileDirPath, undefined, args);
}
var completed = {};
var running = 0;
var stats = {};
...
logger.log('Running', buildTasks.length, 'tasks,', NUM_PROCESSES, 'in parallel', args.filter ? 'with filter ' + JSON.stringify(args.filter) : '...');
...
var collectAnachronisticFiles = function() {
    return collectFilesSync(constants.jsDir,  /\.(pcss|pcssm|jsx)\.js/, true)
        .map(function(fileName) {
            return {
                type: 'f',
                name: fileName.replace(constants.webBuildDir + path.sep, '')
            };
        });
}
...
function updateGeneratedList(buildFilesInfo) {
    ...
    if (args.appDir) {
        ...
        pathPrefix = constants.webDestRelPath;
    }
...
function installBuiltFiles(buildFilesInfo) {
    ...
    if (args.appDir) {
        ...
        destPrefix = constants.webDestDir;
        srcPrefix = constants.webBuildDir;
    }
    logger.log('Installing built files', srcPrefix, '->', destPrefix);
    ...
    if (success) {
        if (args.appDir) {
            if (args.appDir.indexOf(constants.appBuildDir) === 0) {
                ...
                var buildFilesInfo = collectGeneratedFiles(fromDir, relFromDir);
            ...
            //for backwards compatability we copy splunkjs build files to legacy location
            function copySplunkJsFiles(src, dest) {
                mkdirp(dest);
                fs.readdirSync(src).forEach(function(sub) {
                    var subDest = sub === 'index.js' ? 'config.js' : sub;
                    var fullSource = path.join(src, sub);
                    var fullDest = path.join(dest, subDest);
                    logger.debug('copying from', fullSource, 'to', fullDest);
                    copyFile(fullSource, fullDest);
                });
            }
            copySplunkJsFiles(constants.simplexmlSourceDir, constants.simplexmlDestDir);
            copySplunkJsFiles(constants.simplexmlMinSourceDir, constants.simplexmlMinDestDir);
            copySplunkJsFiles(constants.splunkJsSourceDir, constants.splunkJsDestDir);
            copySplunkJsFiles(constants.splunkJsMinSourceDir, constants.splunkJsMinDestDir);

            var buildFilesInfo = concatAll(
                collectGeneratedFiles(constants.simplexmlDestDir, constants.relSimplexmlDestDir),
                collectGeneratedFiles(constants.simplexmlMinDestDir, constants.relSimplexmlMinDestDir),
                collectGeneratedFiles(constants.splunkJsDestDir, constants.relSplunkJsDestDir),
                collectGeneratedFiles(constants.splunkJsMinDestDir, constants.relSplunkJsMinDestDir),
                collectGeneratedFiles(constants.jsBuildDir, constants.relJsBuildDir),
                collectGeneratedFiles(constants.cssDir, constants.relCssDir),
                collectGeneratedFiles(constants.splunkJsCssDir, constants.relSplunkJsCssDir)
            );
            installBuiltFiles(buildFilesInfo);
            if (args.updateGeneratedList) {
                updateGeneratedList(buildFilesInfo);
                updateGeneratedList(collectAnachronisticFiles());
            }
        }
