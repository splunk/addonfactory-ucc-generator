...
@pytest.mark.parametrize(
    ...
    [
        (
                ["build", "--config", "path/to/config.json"],
                {
                    "config_path": "path/to/config.json",
                },
        ),
        (
                ["build", "--config", "path/to/config.yaml"],
                {
...
@mock.patch("splunk_add_on_ucc_framework.commands.build.generate")
def test_config_argument(mock_ucc_gen_generate, args, expected_args):
    main.main(args)

    args, kwargs = mock_ucc_gen_generate.call_args
    assert kwargs["config_path"] == expected_args["config_path"]
