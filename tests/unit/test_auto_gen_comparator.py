...
@patch("splunk_add_on_ucc_framework.commands.build.logger")
def test_deduce_common_conf_xml_file(mock_logger, tmp_path):
    ...
    comparator.deduce_gen_and_custom_content(mock_logger, ignore_file_list=[])
    expected_msg = (
        "---------------------------------------------------------------------------------------------"
        "---------------------------\n\n"
        ...
        f"1) {src_conf_file}\n\n"
        f"2) {src_xml_file}\n\n"
        "Please refer UCC framework documentation for the latest features that "
        "allows you to remove the above files.\n\n"
        "---------------------------------------------------------------------------------------------"
        "---------------------------"
    )
...
@patch("splunk_add_on_ucc_framework.commands.build.logger")
def test_deduce_different_conf_file(mock_logger, tmp_path):
    src_dir = tmp_path / "src"
    dst_dir = tmp_path / "dst"
...
@patch("splunk_add_on_ucc_framework.commands.build.logger")
def test_deduce_different_content_in_conf_file(mock_logger, tmp_path):
    src_dir = tmp_path / "src"
    dst_dir = tmp_path / "dst"
...
@patch("splunk_add_on_ucc_framework.commands.build.logger")
def test_xml_invalid_detection(mock_logger, tmp_path):
    src_dir = tmp_path / "src"
    dst_dir = tmp_path / "dst"
