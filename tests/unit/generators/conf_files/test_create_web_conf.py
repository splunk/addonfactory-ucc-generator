...
def test_set_attributes(global_config, input_dir, output_dir,ucc_dir,ta_name):
    web_conf = WebConf(global_config, input_dir, output_dir, ucc_dir=ucc_dir, addon_name=ta_name)

    # Mock the _gc_schema attribute that contains the endpoints
    web_conf._gc_schema = MagicMock()
    web_conf._gc_schema.endpoints = {"test":"endpoint1"}

    web_conf._set_attributes()

    # Check that the endpoints attribute is set correctly
...
@patch(
    "splunk_add_on_ucc_framework.generators.conf_files.WebConf.get_file_output_path"
)
