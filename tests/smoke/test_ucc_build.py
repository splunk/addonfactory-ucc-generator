...
def test_ucc_generate_with_everything(caplog):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        expected_msg_for_same_file = (
            "Below are the file(s) that are auto generated by the UCC framework. "
            "The below files can be removed from your repository:\n"
            f"1) {dir_path}/../testdata/test_addons/package_global_config_everything/"
            "package/default/restmap.conf\n"
            "Please refer UCC framework documentation for the latest features that allows you "
            "to remove the above files.\n"
        )

        for line in expected_msg_for_same_file.strip().splitlines():
...
def test_check_ucc_ui_files():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        dist_folder = path.join(temp_dir, "dist")
