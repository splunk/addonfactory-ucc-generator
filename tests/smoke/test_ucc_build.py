...
def test_ucc_generate(caplog):
    ...
    for message in caplog.messages:
        exp_msg = re.compile(
            r".*(Below are the file\(s\) that are auto generated by the UCC framework\. The below files can be "
            r"removed from your repository:.+Please refer UCC framework documentation for the latest features "
            r"that allows you to remove the above files\.).*",
            flags=re.DOTALL,
        )
        if exp_msg.search(message):
            # the warning log should be present due to a duplicate file already generated by UCC framework
            assert True
            break
    else:
        assert False
...
def test_ucc_generate_with_everything_uccignore(caplog):
    ...
    for message in caplog.messages:
        ...
        if exp_msg.search(message) is None:
            # the warning log should be absent due to no duplicate files
            assert True
        else:
            assert False
