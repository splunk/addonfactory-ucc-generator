...
class ChainOf(Converter):
    ...
    def decode(self, value, response):
        import copy
        converters = copy.copy(self._converters).reverse()
...
class UserDefined(Converter):
    ...
    def __init__(self, encoder, decoder=None, *args, **kwargs):
        ...
        self._encoder, self._decoder = encoder, decoder or self._encoder
...
class Unifier(Normaliser):
    ...
    def __init__(self, value_map, default=None, case_sensitive=False):
        """

        :param value_map: {"<unified value>": "<original value list>"}
        :param default: default value for input not in specific list
        :param case_sensitive: if it is False, it will return lower case
        """
...
class Boolean(Unifier):
    ...
    Normalize given value to boolean: 0 or 1 (for False and True respectively).
...
class Mapping(Converter):
    ...
    def __init__(self, value_map, case_sensitive=False):
        ...
        for interface, storage in value_map.iteritems():
            if not self._case_sensitive:
                interface, storage = interface.lower(), storage.lower()
            temp = 'Converter "Mapping" supports one-to-one mapping only: "%s"'
    ...
    def encode(self, value, request):
        interface = value.lower() if not self._case_sensitive else value
        return self._map_interface.get(interface, value)

    def decode(self, value, response):
        storage = value.lower() if not self._case_sensitive else value
