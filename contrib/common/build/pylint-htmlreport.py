...
"""
Turns a pylint xml file into a browsable html report along
...
HTML_FOOTER = """
      ...
      <div id="footer">
        <p>
         pylint %s - Pylint is a Python source code analyzer which looks for programming errors, helps enforcing a coding standard and sniffs for some code smells</br>
         </br>
         Internet: <a href="http://www.pylint.org">http://www.pylint.org</a></br>
        <p>
      </div>
  </body>
</html>
...
class PylintHandler(XmlContentHandler):

    """Parses the pylint xml file and produces a list of all its errors."""
...
def generate_pylint_report(source_dir, splunk_home):
    if not os.path.exists(os.path.join(source_dir, 'package', 'bin', '__init__.py')):
        os.mknod(os.path.join(source_dir, 'package', 'bin', '__init__.py'))

    os.chdir(source_dir)
    os.environ['PYTHONPATH'] = os.path.join(source_dir, 'package', 'bin') + ":" + os.path.join(splunk_home, 'lib/python2.7/site-packages')
    os.system('pylint package/bin > pylint.log')
...
if __name__ == '__main__':
    ...
    parser.add_option('--splunk-home', dest='splunk_home',
                      help='The location of Splunk')
    ...
    generate_pylint_report(source_dir, splunk_home)
    ...
    try:
        ...
        print('Failed to parse pylint xml file: %s' % msg)
        sys.exit(1)
    ...
    with io.open(os.path.join(options.report_dir, 'index.html'), 'w') as output_file:
