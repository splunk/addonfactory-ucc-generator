...
Meta
====
    $Id: //splunk/solutions/common/test/InstallUtil.py#12 $
...
class InstallUtil:
    ...
    def __init__(self, soln, splunk_home, logger=logging.getLogger('InstallUtil')):
        """
    ...
    def get_solution(self, release=None, major_version=None, minor_version=None):
        ...
        if(release is "latest"):
            ...
            if sys.platform.startswith("win"):
                # If we are on Windows then copy from this UNC path
                copyfrom_dir = "\\\\sc-build.sv.splunk.com\\bamboo\\builds\\" + self.soln
            else:
                # If we are on Linux then copy from this local path
    ...
    def get_ta_solution(self, release=None, major_version=None, minor_version=None):
        ...
        """ Install latest released build -- http://ta-build.sv.splunk.com:8081/releases/$app/latest/
            Install specific released build -- http://ta-build.sv.splunk.com:8081/releases/$app/releases/major_version/minor_version
            Install latest build -- http://ta-build.sv.splunk.com:8081/builds/"""
        if(release is "latest"):
