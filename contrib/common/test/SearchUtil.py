...
Meta
====
    $Id: //splunk/solutions/common/test/SearchUtil.py#12 $
...
class SearchUtil:
    ...
    def checkQueryContainsRegex(self, key, query, hostPath, namespace, field, regex, saved=False, interval=15, retries = 4, number_results=100):
        tryNum = 0
        tryResults = 0
        r = re.compile(regex)
        ...
        while tryNum <= retries:
            if saved == False:
                sid, results, messages = search_util.getSearchResults(query=query, sessionKey=key, namespace=namespace, hostPath=hostPath)
            else:
                sid, results, messages = search_util.getSavedSearchResults(query=query, sessionKey=key, namespace=namespace, hostPath=hostPath)
            for result in results:
                self.logger.debug(result)
                self.logger.debug(result[field])
                if tryResults > number_results:
    ...
    def checkQueryCount(self, key, query, targetCount, hostPath, namespace, saved=False, interval=15, retries = 4):
        ...
        while tryNum <= retries:
            if saved == False:
                result = search_util.checkQueryCounts(query=query, expectedEventCnt=targetCount, key=key, namespace=namespace, hostPath=hostPath)
            else:
                result = search_util.checkSavedQueryCounts(query=query, expectedEventCnt=targetCount, key=key, namespace=namespace, hostPath=hostPath)
            if result['match']:
    ...
    def checkQueryCountIsGreaterThanZero(self, key, query, hostPath, namespace, saved=False, interval=15, retries = 4):
        tryNum = 0
        search_util = SearchTestUtil()
        while tryNum <= retries:
            if saved == False:
                ...
                sid, results, messages = search_util.getSavedSearchResults(query=query, sessionKey=key, namespace=namespace, hostPath=hostPath)
            if len(results) > 0:
                self.logger.debug("Count of results is > 0, it is:%d", len(results))
                return True
            else:
    ...
    def queryCountsEqual(self, key, query, query2, hostPath, namespace, interval=15, retries=4):
        tryNum = 0
        search_util = SearchTestUtil()
        while tryNum <= retries:
            sid, results, messages = search_util.getSearchResults(query=query, sessionKey=key, namespace=namespace, hostPath=hostPath)
            sid2, results2, messages2 = search_util.getSearchResults(query=query, sessionKey=key, namespace=namespace, hostPath=hostPath)
            self.logger.debug("messages from search '%s': %s", query, messages)
            self.logger.debug("messages from search '%s': %s", query2, messages2)
            if len(results) == len(results2):
                return True
            else:
                self.logger.debug("Count of results for query %s is %d", query, len(results))
                self.logger.debug("Count of results for query2 %s is %d", query, len(results2))
                tryNum += 1
                time.sleep(interval)
        return False

    def nonZeroQueryCountsEqual(self, key, query, query2, hostPath, namespace, interval=15, retries=4):
        tryNum = 0
        search_util = SearchTestUtil()
        while tryNum <= retries:
            sid, results, messages = search_util.getSearchResults(query=query, sessionKey=key, namespace=namespace, hostPath=hostPath)
            ...
            self.logger.debug("messages from search '%s': %s", query2, messages2)
            if len(results) > 0:
                self.logger.debug("Count of results for query %s is > 0, it is:%d", query, len(results))
                if len(results2) > 0:
                    self.logger.debug("Count of results for query %s is > 0, it is:%d", query2, len(results))
                    if len(results) == len(results2):
                            return True
            else:
                self.logger.debug("Count of results for query %s is 0", query)
                tryNum += 1
                time.sleep(interval)
        return False

    def getRealtimeNotableSearchResults(self, searchName, remote_key, interval=15, retries=4, minimumNumberEvents=1):
        ...
        while tryNum <= retries:
            ...
            searchResults = search_util.getSearchResults(searchQuery, remote_key, isRealTimeSearch=False)
            jobid, results, jobmessages = searchResults
            self.logger.debug("Results Count: %d", len(results))
            if len(results) >= minimumNumberEvents:
    ...
    def checkQueryContent(self,
        ...
        while tryNum <= retries and not status:
            sid, results, messages = search_util.getSearchResults(query=query,
                sessionKey=key,
                namespace=namespace)
    ...
    def checkQueryFieldValues(self,
        ...
        while tryNum <= retries and not status:
            ...
            if len(results) > 0:
                ...
                values = str(results[expectedMinRow].values())
                print values
    ...
    def checkGapSearch(self,
        ...
        while tryNum <= retries and status:
            ...
            self.logger.debug("Ran Gap Detection search=\"%s\" with sid=%s and warningGapCountLimit=%s and warningGapSizeLimit=%s" , query, sid, str(warningGapCountLimit), str(warningGapSizeLimit))
            ...
            if len(results) > 0:
                ...
                for result in results:
                    if float(str(result["delta"])) <= warningGapSizeLimit:
                        ...
                        warningCount += 1
                        self.wrapLogOutput(msg='Warning Sized Gap detected with detection search sid=' + str(sid),
                        ...
                        errorCount += 1
                        self.wrapLogOutput(msg='Error Sized Gap detected with detection search sid=' + str(sid),
                            actual   = "gapSize=" + str(result["delta"]),
                ...
                if warningCount >= warningGapCountLimit:
                    ...
                    status = False
                    self.wrapLogOutput(msg='Too many warning sized gaps detected with detection search sid=' + str(sid),
                    ...
                    self.wrapLogOutput(msg='Safe number of warning sized gaps detected with detection search sid=' + str(sid),
                            actual   = "warningGapCount=" + str(warningCount),
                            expected = "warningGapCountLimit=" + str(warningGapCountLimit),
                            errors   = '',
                            level    = 'warning')
                else:
                    self.wrapLogOutput(msg='No warning sized gaps detected with detection search sid=' + str(sid),
                            actual   = "warningGapCount=" + str(warningCount),
                            expected = "warningGapCountLimit=" + str(warningGapCountLimit),
                            errors   = '',
                            level    = 'debug')
        ...
        self.wrapLogOutput(msg='Error-Sized Gap Detection Results for search sid=' + str(sid),
    ...
    def checkFieldAgainstCanon(self,
        ...
        while tryNum <= retries and not status:
            ...
            self.logger.debug("Ran canon test search=\"%s\" with sid=%s" , query, sid)
            ...
            if len(results) > 0:
                ...
                self.logger.debug("No results from canon test search with sid=%s retrying after wait interval... (unless max tries exceeded)", sid)
    ...
    def checkExactQueryContentJson(self,
                          key,
                          query,
        ...
        '''Check for exact content in a specific search result.
           Script will issue failure IF no results are obtained
           ( len(result) == 0 ) OR if the result text does not match
           the expected text.
        ...
        try:
            path_to_output = os.path.abspath(expected)
        ...
        while tryNum <= retries and not status:
            sid, results, messages = search_util.getSearchResults(query=query,
                sessionKey=key,
                namespace=namespace, hostPath=hostPath)
            if len(results) > 0:
                # Careful when using _raw as it is a splunk.search.RawEvent,
                # not a string. Casting to a string converts it to a
                # lxml.etree._ElementStringResult which behaves like a string.
                ...
                for result in results:
                    actual.append(','.join(map(str, result.values())) + '\n')
                expected = json.loads(output.readline())
                output.seek(0)
                status   = self.compareContent(actual, expected)
            else:
                self.wrapLogOutput(msg='Zero results from search:',
    ...
    def saveQueryContent(self,
        ...
        '''Save the exact content in a specific search result.
           Script will issue failure IF no results are obtained
           ( len(result) == 0 )
           This function is for use only by tests which do NOT
           require passing input to Splunk on STDIN.
        ...
        try:
            ...
            self.logger.debug('fail to open output file '+path_to_output)
        ...
        while tryNum <= retries and not status:
            ...
            if len(results) > 0:
                ...
                for result in results:
                    actual.append(','.join(map(str, result.values())) + '\n')
                output.write(json.dumps(actual))
                status = True
            else:
                self.wrapLogOutput(msg='Zero results from search:',
                    actual   = '',
    ...
    def checkQueryStatsCountEquals(self,
                                 key,
                                 query,
                                 expected,
                                 hostPath,
        ...
        while tryNum <= retries:
            sid, results, messages = search_util.getSearchResults(query=query, sessionKey=key, namespace=namespace, hostPath=hostPath)
            count = int(str(results[0]['count']))
            if count >= expected:
                self.logger.debug("Count of results is >= %d, it is:%d", expected, count)
                return True
            else:
                self.logger.debug("Count of results test fails")
                tryNum += 1
                time.sleep(interval)
        return False


    def checkQueryStatsCountIsNotLess(self,
                                 key,
                                 query,
                                 expected,
                                 hostPath,
    ...
    def checkQueryStatsCountIsInRange(self,
                                 key,
                                 query,
                                 lowerBound,
                                 upperBound,
        ...
        while tryNum <= retries:
            ...
            if count >= lowerBound and count <=upperBound:
                self.logger.debug("Count of results is between %d and %d, it is:%d", lowerBound, upperBound, count)
                return True
            else:
                self.logger.debug("Count of results test fails")
                tryNum += 1
