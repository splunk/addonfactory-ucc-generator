...
class ServerAppFetcher(LocalAppFetcher):
    ...
    def get_remote_apps(self):
        ...
        if self.parse_args.appname:
            ...
            #Eg: ('ES', 'latest')
            app_details = map(None, self.parse_args.appname, self.parse_args.appversion, self.parse_args.build_numbers)

            DELIVER_AS = "DELIVER_AS=file" + "&"

            for (app, specific_version, build_number) in app_details:

                #TA-* apps reside on ta-build-sec.
                ...
                if self.parse_args.artifactorypull:
                    ...
                    app_url = self.get_artifactory_url(app, build_number=build_number, specific_version=specific_version)
    ...
    def get_artifactory_url(self, app_name, build_number=None, specific_version=None):
