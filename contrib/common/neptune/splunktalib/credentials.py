...
def create_credential_manager(username, password, splunkd_uri, app, owner,
                              realm):
    session_key = CredentialManager.get_session_key(username, password,
                                                    splunkd_uri)
...
class CredentialManager(object):
    ...
    def __init__(self,
        ...
        self._owner = owner
        self._sep = "``splunk_cred_sep``" if sep is None else sep
    ...
    def _create(self, name, str_to_encrypt):
        ...
        if not resp or resp.status not in (200, 201):
            raise CredException("Failed to encrypt username {} {} {}".format(
                name, resp.status, content))
    ...
    def get_all_passwords(self):
        ...
        raise CredException("Failed to get credentials %s %s", response.status,
                            content)
    ...
    def _build_name(realm, name):
        return urllib.quote("".join((CredentialManager._escape_string(
            realm), ":", CredentialManager._escape_string(name), ":")))
