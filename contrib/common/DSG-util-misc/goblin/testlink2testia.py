...
class Goblin():
    ...
    def create_testia_xml(self, test_cases):
        output_file = open('cases.csv', 'w')
        output_file.write('Case Id;Title;Date;Priority;Planned duration (minutes);Test Areas;Average Duration;Tags;Objective;Test Data;Preconditions & Assumptions\r\n')
        for test_suite in sorted(test_cases.keys()):
            tag = ''.join([s.title() for s in test_suite.split('_')])
            cases = test_cases[test_suite]
            if len(cases) > 0:
                for case in cases:
                    output_file.write('new;')
                    title = ' '.join(case['name'].split('_')).capitalize().strip()
                    output_file.write(title + ';2016-07-18;0;;"";0;' + tag + ';"";"";""\r\n')
                    output_file.write('"";Step Id;Action;Result\r\n')
                    if len(case['steps']) > 0:
                        for step in case['steps']:
                            output_file.write('"";new;')
                            action = step['actions'].encode('ascii', 'replace').strip()
                            if len(action) == 0:
                                action = 'Dummy action'
                            result = step['expected_results'].encode('ascii', 'replace').strip()
                            if len(result) == 0:
                                result = 'Dummy result'
                            output_file.write('"' + action + '";')
                            output_file.write('"' + result + '"\r\n')
                    else:
                        output_file.write('"";new;Dummy action;Dummy result\r\n')
    ...
    if ta_name == myl.SEARCH_ROOT_NAME:
        ...
        for id1 in nodes.keys():
            if nodes[id1]['name'] == ta_name:
                myl.create_testia_xml(myl.get_test_cases(id1, ''))
